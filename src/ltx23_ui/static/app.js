...
function modelData() {
  return {
    ...
    quantization:$('quantization').value, offload:$('offload').value, max_batch_size:n('maxBatch')
  };
}
function requestData() {
  return {model:modelData(), generation:{
    ...
    enhance_prompt:$('enhancePrompt').checked, guidance:{cfg_scale:n('cfg'),stg_scale:n('stg'),rescale_scale:n('rescale'),a2v_scale:n('a2v'),skip_step:n('skipStep'),stg_blocks:$('stgBlocks').value.split(',').map(x=>Number(x.trim())).filter(Number.isFinite)}
  }};
}
...
function showProgress(job){$('progressBox').classList.remove('hidden');$('progressMessage').textContent=job.message;$('progressValue').textContent=`${job.progress}%`;$('progressBar').style.width=`${job.progress}%`}
